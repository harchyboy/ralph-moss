...
function New-RalphPullRequest {
    ...
    try {
        ...
        try {
            $null = git push -u origin $CurrentBranch 2>&1
        } catch {
            # Ignore push errors - might already be pushed
        }

        # Check if PR already exists
        try {
            $existingPRJson = & gh pr view $CurrentBranch --json url 2>&1
            if ($existingPRJson -match '"url"') {
...
"@
        ...
        # Create PR using body file to avoid escaping issues
        $bodyFile = Join-Path $WorkDir "pr-body.tmp"
        $prBody | Set-Content -Path $bodyFile -Encoding UTF8

        $prArgs = @("pr", "create", "--title", $prTitle, "--body-file", $bodyFile, "--base", "main")
        if ($Draft) {
            $prArgs += "--draft"
        }

        # Run gh pr create - use Start-Process to avoid stderr throwing exceptions
        $tempOutput = Join-Path $WorkDir "pr-output.tmp"
        $tempError = Join-Path $WorkDir "pr-error.tmp"

        $processArgs = "pr create --title `"$prTitle`" --body-file `"$bodyFile`" --base main"
        if ($Draft) {
            $processArgs += " --draft"
        }

        $process = Start-Process -FilePath "gh" -ArgumentList $processArgs -NoNewWindow -Wait -PassThru -RedirectStandardOutput $tempOutput -RedirectStandardError $tempError
        $prExitCode = $process.ExitCode

        $prOutput = if (Test-Path $tempOutput) { Get-Content $tempOutput -Raw } else { "" }
        $prError = if (Test-Path $tempError) { Get-Content $tempError -Raw } else { "" }

        # Cleanup temp files
        Remove-Item $tempOutput -Force -ErrorAction SilentlyContinue
        Remove-Item $tempError -Force -ErrorAction SilentlyContinue

        # Only show errors (not warnings about uncommitted changes)
        if ($prError -and $prError -notmatch "Warning:") {
            Write-Host "  [PR] Error: $prError" -ForegroundColor Yellow
        }

        # Cleanup body temp file
        Remove-Item $bodyFile -Force -ErrorAction SilentlyContinue

        # Check if we got a URL back
        $prOutput = $prOutput.Trim()
        if ($prOutput -match "https://github\.com/[^\s]+") {
            $prUrl = $Matches[0]
            ...
            return $prUrl
        } elseif ($prExitCode -eq 0 -and $prOutput) {
            # Sometimes gh returns just the URL
            Write-Host "  [PR] Created: $prOutput" -ForegroundColor Green
            $prd | Add-Member -NotePropertyName "prUrl" -NotePropertyValue $prOutput -Force
            $prd | ConvertTo-Json -Depth 10 | Set-Content $PrdFile
            return $prOutput
        } else {
            $errorMsg = if ($prError) { $prError } else { "Unknown error" }
            Write-Warning "  [PR] Failed to create PR (exit code: $prExitCode): $errorMsg"
            return $null
        }
